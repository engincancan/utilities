...
#else  /* __ICorProfilerInfo2_INTERFACE_DEFINED__ */

#if defined(_X86_)
            hr = m_pProfilerInfo->SetEnterLeaveFunctionHooks ( (FunctionEnter *)&EnterStub,
                                                               (FunctionLeave *)&LeaveNaked,
                                                               (FunctionTailcall *)&TailcallStub );
#else

#error      Can only compile 64-bit for version 2.0 of the CLR
...
HRESULT  ProfilerCallback::GarbageCollectionFinished()
{
    ...
    _GenerationBounds(FALSE, FALSE, m_condemnedGeneration);
...
void ProfilerCallback::_ProcessEnvVariables()
{
    ...
    if ( m_dwMode & (DWORD)OBJECT )
    {
        ...
        if ( m_classToMonitor != NULL )
        {
            _snwprintf_s( m_classToMonitor, len, len-1, L"%S", buffer );
        }
        else
        {
            //
    ...
    //
    DWORD value2 = BASEHELPER::FetchEnvironment( OMV_INITIAL_SETTING );
...
HRESULT ProfilerCallback::_InitializeNamesForEventsAndCallbacks()
{
    ...
    for ( DWORD i=GC_HANDLE; ( (i<SENTINEL_HANDLE) && SUCCEEDED(hr) ); i++ )
    {
        ...
        // allocate space
        //
        SIZE_T namedEventLen = strlen(NamedEvents[i]) + 1 + 9;
        SIZE_T callbackNamedEventLen = strlen(CallbackNamedEvents[i])+1+9;
        ...
        {
            sprintf_s( m_NamedEvents[i], namedEventLen, "%s_%08x", NamedEvents[i], m_dwProcessId );
            sprintf_s( m_CallbackNamedEvents[i], callbackNamedEventLen, "%s_%08x", CallbackNamedEvents[i], m_dwProcessId );
        }
        else
            hr = E_FAIL;
    }
            ...
            if ( fSuccess == TRUE )
            {
                ...
                { 
                    ...
                    if ( (cbRead == 0) || m_logFileName[0] == NULL )
                    {
                        ...
                        sprintf_s( m_logFileName, ARRAY_LEN(m_logFileName), "pipe_%08x.log", m_dwProcessId );
                        break;
                    }
